...
__align(4) u8 dtbuf[50];  
	u8 upload=1;u16 i,rxlen;	u8 key=0XFF;
...
void Gps_Msg_Show(void)
{
 ...
 //	LCD_ShowString(30,140,200,16,16,dtbuf);	 	 
  ...
  printf("Speed:%.3fkm/h     ",tp/=1000);
	...
	printf("GPS+BD Valid satellite:%02d",gpsx.posslnum);
 ...
 //	LCD_ShowString(30,240,200,16,16,dtbuf);
	printf("GPS Visible satellite:%02d",gpsx.svnum%100);
	sprintf((char *)dtbuf,"BD Visible satellite:%02d",gpsx.beidou_svnum%100);	 		//可见北斗卫星数
 //	LCD_ShowString(30,260,200,16,16,dtbuf);
	printf("BD Visible satellite:%02d",gpsx.beidou_svnum%100);

	sprintf((char *)dtbuf,"UTC Date:%04d/%02d/%02d   ",gpsx.utc.year,gpsx.utc.month,gpsx.utc.date);	//显示UTC日期
		printf("UTC Date:%04d/%02d/%02d   ",gpsx.utc.year,gpsx.utc.month,gpsx.utc.date);
