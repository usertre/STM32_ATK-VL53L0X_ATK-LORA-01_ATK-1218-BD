...
void USART2_IRQHandler(void)
{
	...
	res =USART_ReceiveData(USART2);		
		if(USART2_RX_STA<USART2_MAX_RECV_LEN)		//还可以接收数据
		{
			...
			if(USART2_RX_STA==0)TIM7_Set(1);	 	//使能定时器4的中断 
...
{  
 ...
 //	USART2->BRR=(pclk1*1000000)/(bound);// 波特率设置	 
	...
	UART_DMA_Config1(DMA1_Channel7,(u32)&USART2->DR,(u32)USART2_TX_BUF);//DMA1通道7,外设为串口2,存储器为USART2_TX_BUF 
...
{  
	...
	UART_DMA_Enable1(DMA1_Channel7,strlen((const char*)USART2_TX_BUF)); 	//通过dma发送出去
}
//定时器4中断服务程序		    
